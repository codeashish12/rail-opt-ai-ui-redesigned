...
export function DashboardHomeRedesigned({ setActive }: Props) {
  ...
  const maintenanceCompletion = resultMetrics?.maintenanceCompletion ?? (
    maintenanceRequests.length
      ? Math.round(
          maintenanceRequests.filter((request) => request.status === 'Approved').length /
            maintenanceRequests.length *
            100,
        )
      : 0
  )
  const assetAvailability = resultMetrics?.assetAvailability ?? (
    resources.length
      ? Math.round(
          resources.filter((resource) => resource.availability === 'Available').length /
            resources.length *
            100,
        )
      : 0
  ...
  const recommendedSection = sections.find(
    (section) => section.id === recommendedRequest?.section,
  ) ?? selected
  ...
  const timelineRows = sections.map((section, index) => {
    const sectionTrains = trains.filter((train) => train.section === section.id)
    const first = sectionTrains[0]
    const start = first ? timeToMinutes(first.arrival) : 60 + index * 70
    const duration = Math.max(
      55,
      first ? timeToMinutes(first.departure) - timeToMinutes(first.arrival) : 75,
    )
    return {
      section,
      start,
  ...
  return (
    <div data-dashboard-page className="motion-safe:animate-in motion-safe:fade-in motion-safe:slide-in-from-bottom-2 motion-safe:duration-500">
      {/* Hero */}
      ...
      <div className="grid gap-3 sm:grid-cols-2 xl:grid-cols-3 2xl:grid-cols-6">
        ...
        <RailKpi
          ...
          change={result ? 'Live optimizer result' : 'Generate a plan to populate'}
        ...
        <RailKpi
          ...
          status={selected && selected.utilization >= 85 ? 'warning' : 'success'}
          className="motion-safe:transition-all motion-safe:duration-200 motion-safe:hover:-translate-y-0.5 motion-safe:hover:shadow-md"
        />
        <RailKpi
      ...
      <div className="mt-5 grid gap-5 xl:grid-cols-[minmax(0,1.25fr)_minmax(360px,0.75fr)]">
          ...
          <div className="flex flex-col justify-between gap-3 border-b border-border px-5 py-4 md:flex-row md:items-center">
            <div>
              <div className="flex items-center gap-2">
                ...
                <h3 className="font-mono text-sm font-semibold">Network Overview</h3>
              </div>
          ...
          <div className="overflow-x-auto px-5 py-8">
            <div className="relative min-w-[720px]">
              ...
              <div className="relative flex items-start justify-between">
                {stations.slice(0, Math.max(5, sections.length + 1)).map((station, index) => {
                  const section = sections[index % Math.max(1, sections.length)]
                  const isSelected = section?.id === selectedSection

                  return (
                    <div key={station.code} className="flex w-28 flex-col items-center">
                      <button
                        ...
                        title={section ? `${section.from} → ${section.to}` : station.name}
                      >
                        {station.code}
                      </button>
          ...
          {selected && (
            <div className="border-t border-border bg-muted/20 px-5 py-4">
              <div className="grid gap-4 sm:grid-cols-4">
                <div>
                  <p className="text-[9px] uppercase tracking-[0.14em] text-muted-foreground">Section</p>
                  <p className="mt-1 font-mono text-xs font-semibold">{selected.id}</p>
                </div>
                <div>
                  <p className="text-[9px] uppercase tracking-[0.14em] text-muted-foreground">Corridor</p>
                  <p className="mt-1 text-xs font-medium">{selected.from} → {selected.to}</p>
                </div>
                <div>
                  <p className="text-[9px] uppercase tracking-[0.14em] text-muted-foreground">Trains</p>
                  <p className="mt-1 font-mono text-xs font-semibold">{selected.trains}</p>
                </div>
                <div>
                  <p className="text-[9px] uppercase tracking-[0.14em] text-muted-foreground">Utilization</p>
                  <p className="mt-1 font-mono text-xs font-semibold">{selected.utilization}%</p>
                </div>
              </div>
            </div>
          )}
        ...
        <section className="overflow-hidden rounded-xl border border-primary/25 bg-card motion-safe:animate-in motion-safe:fade-in motion-safe:slide-in-from-bottom-2 motion-safe:duration-500">
          <div className="border-b border-border px-5 py-4">
            ...
            <p className="mt-1 text-xs text-muted-foreground">Highest-value work for the next operating window</p>
          </div>
          ...
          <div className="p-5">
            <div className="rounded-lg border border-primary/20 bg-primary/5 p-5">
              ...
              <p className="mt-4 font-mono text-3xl font-semibold tracking-tight">
                {upcomingBlocks[0]?.time? formatBlockTime(upcomingBlocks[0].time): '—'}
              </p>

              <div className="mt-4 grid grid-cols-2 gap-4">
                <div>
                  <p className="text-[9px] uppercase tracking-[0.14em] text-muted-foreground">Location</p>
                  <p className="mt-1 text-xs font-semibold">
                    {recommendedSection?.from ?? '—'} — {recommendedSection?.to ?? '—'}
                  </p>
                </div>
                <div>
                  <p className="text-[9px] uppercase tracking-[0.14em] text-muted-foreground">Block Type</p>
            ...
            <div className="mt-4 grid grid-cols-2 gap-2 sm:grid-cols-4">
              {[
                ['Jobs', String(Math.max(1, activeRequests.length ? Math.min(4, activeRequests.length) : 1)), Wrench],
                ...
                <div key={String(label)} className="rounded-md border border-border bg-background p-3">
                  ...
                  <p className="mt-3 font-mono text-lg font-semibold">{String(value)}</p>
      ...
      <div className="mt-5 grid gap-5 xl:grid-cols-[minmax(0,1.4fr)_360px]">
        <section className="overflow-hidden rounded-xl border border-border bg-card motion-safe:animate-in motion-safe:fade-in motion-safe:slide-in-from-bottom-2 motion-safe:duration-500">
          <div className="flex items-center justify-between border-b border-border px-5 py-4">
            <div>
              <h3 className="font-mono text-sm font-semibold">Block Plan Timeline</h3>
              <p className="mt-1 text-xs text-muted-foreground">
                Train movements and maintenance possessions across the next 24 hours.
              </p>
            </div>
            <button
          ...
          <div className="overflow-x-auto p-5">
            <div className="min-w-[820px]">
                  ...
                  return (
                    <div key={row.section.id} className="flex items-center gap-3">
                      <div className="w-37 shrink-0">
                        <p className="font-mono text-[10px] font-semibold">{row.label}</p>
                        <p className="mt-1 text-[9px] text-muted-foreground">
              ...
              <div className="mt-5 flex flex-wrap gap-4 text-[9px] text-muted-foreground">
                <span className="flex items-center gap-1.5"><i className="size-2 rounded-sm bg-primary" /> Planned</span>
                <span className="flex items-center gap-1.5"><i className="size-2 rounded-sm bg-accent" /> Maintenance</span>
                <span className="flex items-center gap-1.5"><i className="size-2 rounded-sm bg-destructive" /> Protected / conflict</span>
              </div>
            </div>
          </div>
        </section>
        ...
        <div className="space-y-5">
          <section className="rounded-xl border border-border bg-card motion-safe:animate-in motion-safe:fade-in motion-safe:slide-in-from-bottom-2 motion-safe:duration-500">
            <div className="border-b border-border px-5 py-4">
              <div className="flex items-center justify-between">
                <div>
                  <h3 className="font-mono text-sm font-semibold">Upcoming High Priority Blocks</h3>
                  <p className="mt-1 text-xs text-muted-foreground">Work requiring planner attention</p>
                </div>
              ...
              {upcomingBlocks.map((block, index) => (
                <div key={`${block.id}-${index}`} className="p-4">
                  <div className="flex items-start justify-between gap-3">
                    <div>
                      <p className="font-mono text-xs font-semibold">{block.section}</p>
                      <p className="mt-1 text-[10px] text-muted-foreground">{block.time}</p>
                      <p className="mt-2 text-[10px] text-muted-foreground">{block.tasks}</p>
                    </div>
          ...
          <section className="rounded-xl border border-border bg-card motion-safe:animate-in motion-safe:fade-in motion-safe:slide-in-from-bottom-2 motion-safe:duration-500">
            <div className="border-b border-border px-5 py-4">
              <h3 className="font-mono text-sm font-semibold">Alerts & Notifications</h3>
              <p className="mt-1 text-xs text-muted-foreground">Signals that can affect the plan</p>
            </div>

            <div className="space-y-3 p-4">
              {resources.filter((resource) => resource.conflict).slice(0, 3).map((resource) => (
                <div key={resource.id} className="flex gap-3 rounded-md border border-destructive/20 bg-destructive/5 p-3">
                  <AlertTriangle className="mt-0.5 size-3.5 shrink-0 text-destructive" />
                  <div className="min-w-0">
                    <p className="text-[10px] font-semibold">{resource.id} resource conflict</p>
                    <p className="mt-1 text-[9px] leading-4 text-muted-foreground">{resource.conflict}</p>
                  </div>
                </div>
              ))}
              ...
              {!resources.some((resource) => resource.conflict) && (
                <div className="flex gap-3 rounded-md border border-primary/20 bg-primary/5 p-3">
                  ...
                  <p className="text-[10px] text-muted-foreground">No resource conflicts are currently flagged.</p>
                </div>
              )}
              ...
              <div className="flex gap-3 rounded-md border border-border bg-muted/20 p-3">
                ...
                <p className="text-[10px] text-muted-foreground">
                  {activeRequests.length} active maintenance requests are waiting for scheduling.
                </p>
              </div>
            </div>
          </section>
      ...
      <div className="mt-5 grid gap-5 xl:grid-cols-[minmax(0,1fr)_minmax(360px,0.8fr)]">
        <section className="rounded-xl border border-border bg-card motion-safe:animate-in motion-safe:fade-in motion-safe:slide-in-from-bottom-2 motion-safe:duration-500">
          <div className="border-b border-border px-5 py-4">
            <div className="flex items-center justify-between gap-3">
              <div>
                <h3 className="font-mono text-sm font-semibold">Optimization Impact</h3>
                <p className="mt-1 text-xs text-muted-foreground">
                  Compare the current optimizer result against manual planning assumptions.
                </p>
              </div>
          ...
          <div className="grid grid-cols-2 divide-x divide-y divide-border sm:grid-cols-3 xl:grid-cols-6">
            {[
              ['Blocks Required', String(totalBlocks), result ? 'Live result' : 'Pending'],
              ['Maintenance Jobs', String(activeRequests.length), 'Active work'],
              ['Block Hours', `${blockHours}h`, result ? 'Solver output' : 'Pending'],
              ['Train Conflicts', String(trainConflicts), trainConflicts ? 'Review' : 'Clear'],
              ['Asset Availability', `${assetAvailability}%`, 'Resource status'],
              ['Plan Readiness', result ? 'Ready' : 'Not generated', result ? 'Validated' : 'Run optimizer'],
              ...
              <div key={label} className="p-4">
                ...
                <p className="mt-1 text-[9px] text-muted-foreground">{caption}</p>
              </div>
            ))}
          </div>
        </section>
        ...
        <section className="rounded-xl border border-border bg-card motion-safe:animate-in motion-safe:fade-in motion-safe:slide-in-from-bottom-2 motion-safe:duration-500">
          <div className="border-b border-border px-5 py-4">
            <h3 className="font-mono text-sm font-semibold">What-if Simulator · Quick Actions</h3>
            <p className="mt-1 text-xs text-muted-foreground">Stress-test the plan before approval.</p>
          </div>
          ...
          <div className="grid grid-cols-2 gap-2 p-4">
            {[
              ...
              >
                ...
                <p className="mt-3 text-[10px] font-semibold">{String(label)}</p>
                <p className="mt-1 text-[9px] text-muted-foreground">{String(caption)}</p>
              </button>
            ))}
          </div>
        </section>
        ...
        <div className="mt-5 flex items-start gap-3 rounded-xl border border-accent/30 bg-accent/10 p-4 motion-safe:animate-in motion-safe:fade-in motion-safe:slide-in-from-bottom-2 motion-safe:duration-300">
          ...
          <div className="min-w-0">
            ...
            <p className="mt-1 text-[10px] leading-4 text-muted-foreground">
              ...
              resource availability, emergency maintenance, and weather restrictions.
            </p>
          </div>
          <button
            onClick={() => setActive?.('What-if Simulation')}
